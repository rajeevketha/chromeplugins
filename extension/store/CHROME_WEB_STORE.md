...
**Upload package:** build with:

```bash
cd extension && zip -r ../OrgKit-1.6.6-store.zip . -x '*.DS_Store' -x 'store/submission/*'
```

Or use the prebuilt `OrgKit-1.6.6-store.zip` in agent Files / repo artifacts.
